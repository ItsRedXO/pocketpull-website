...
export async function processFirstDepositBonus(_blink:any,userId:string,depositAmt:number){
  try{
    ...
    const bonus=Math.min((Number(depositAmt)||0)*(deal.depositMatchPercent/100),deal.depositMatchCap);
    if(bonus<=0)return;
    ...
    await client.query("INSERT INTO transactions(id,user_id,type,amount,description,source_id,created_at) VALUES($1,$2,'first_deposit_bonus',$3,$4,$5,now())",[id,userId,bonus,`First Deposit Bonus — ${deal.depositMatchPercent}% match (Matched funds) (Deposited ${Number(depositAmt).toFixed(2)}, earned ${bonus.toFixed(2)})`,id]);
...
export async function processReferralReward(_blink:any,userId:string,referrerId:string,depositAmt:number){
  try{
    ...
    const reward=deal.referralRewardAmount;
    ...
    await client.query("INSERT INTO transactions(id,user_id,type,amount,description,source_id,created_at) VALUES($1,$2,'referral_signup_bonus',$3,$4,$5,now())",[referredId,userId,reward,`Referral Signup Bonus — $${reward.toFixed(2)} for using a friend's referral code!`,referredId]);
