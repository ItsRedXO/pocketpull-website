...
/** Auth/email services remain on Blink; all PocketPull application data is PostgreSQL-backed. */
export function getBlinkServer(env: Record<string, string>) {
  ...
  return { auth: blink.auth, notifications: blink.notifications, db: createDatabase(env), realtime: { async publish() { return undefined; } } };
}
...
export function getRewardUserId(originalUserId: string, isBot: boolean): string { if (!originalUserId) return BOT_REWARD_RECIPIENT_ID; if (isBot || originalUserId.startsWith('ai_')) return BOT_REWARD_RECIPIENT_ID; return originalUserId; }
