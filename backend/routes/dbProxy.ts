...
const PUBLIC_READ = new Set(['packsCatalog', 'packCards', 'battles', 'battlePlayers', 'battleParticipants', 'battleResults', 'battlePullAudits', 'leaderboardStats']);
...
function requireScope(logical: string, operation: string, userId: string | null, admin: boolean, options: any = {}) {
  ...
  if (operation === 'create' && WRITE_BLOCKED_FOR_USERS.has(logical)) throw new Error('FORBIDDEN');
  ...
  if (USER_SCOPED.has(logical)) {
    ...
    if (operation === 'get' && options.id) {
      // Ownership is checked after fetching.
      return;
    }
    if (operation === 'create' && options.data?.userId !== userId) throw new Error('FORBIDDEN');
  }
...
async function supportQuery(logical: string, operation: string, options: any, userId: string | null, admin: boolean) {
  ...
  if (operation === 'list') {
    ...
    params.push(limit);
    let sql = `SELECT * FROM ${table}${clauses.length ? ` WHERE ${clauses.join(' AND ')}` : ''}${order ? ` ORDER BY ${order}` : ''} LIMIT $${params.length}`;
...
app.post('/db', async (c) => {
  try {
    ...
    if ((logical === 'supportChats' || logical === 'supportMessages') && ['get', 'list'].includes(operation)) {
    ...
    if (operation === 'upsert') throw new Error('upsert is not exposed through the browser DB proxy');
