...
function normalize(input: Record<string, any>, known: Set<string>): Record<string, any> {
...
function insertSql(table: string, values: Record<string, any>) {
  ...
  return { sql: `INSERT INTO ${table} (${keys.join(',')}) VALUES (${keys.map((_, i) => `$${i + 1}`).join(',')}) RETURNING *`, params: keys.map(key => dbValue(key, values[key])) };
}
...
async function savePack(body: any, adminUserId: string) {
  ...
  return transaction(async client => {
    ...
    const currentQuantity = input.packType === 'mystery' ? totalMysteryQuantity : existing ? (existingQuantityLimit === 0 && qLimit > 0 ? qLimit : Number(existing.current_quantity || 0)) : qLimit;
    const packValues: Record<string, any> = normalize({
      id: packId || `pack_${Date.now()}_${uid().slice(-6)}`,
      packType: input.packType,
      name: String(input.name).trim(), price,
      description: String(input.description || '').trim(), imageUrl: input.imageUrl || null,
      glowColor: input.glowColor || '#00c8ff', borderColor: input.borderColor || input.glowColor || '#00c8ff',
      isActive: input.isActive ? 1 : 0, sortOrder: parseInt(String(input.sortOrder ?? 0), 10) || 0,
      quantityLimit: qLimit, currentQuantity, cooldownHours: parseInt(String(input.cooldownHours ?? 0), 10) || 0,
      expiresAt: input.expiresAt || null, nameColor: input.nameColor || '#ffffff', descriptionColor: input.descriptionColor || '#ffffff',
      priceColor: input.priceColor || '#ffffff', buttonTextColor: input.buttonTextColor || '#ffffff',
      openAnotherButtonTextColor: input.openAnotherButtonTextColor || input.buttonTextColor || '#ffffff',
    }, PACK_COLUMNS);

    if (!existing) {
      packId = String(packValues.id);
      const { sql, params } = insertSql('packs_catalog', packValues);
      await client.query(sql, params);
    } else {
      const data = { ...(existing.data && typeof existing.data === 'object' ? existing.data : {}), ...((packValues.data as Record<string, any>) || {}), adminUpdatedBy: adminUserId, adminUpdatedAt: now.toISOString() };
      const updateValues: Record<string, any> = { ...packValues, data };
    ...
    for (let i = 0; i < cards.length; i++) {
      const card = cards[i];
      const quantity = Math.max(0, parseInt(String(card.quantity ?? 0), 10) || 0);
      const cardValues: Record<string, any> = normalize({
        id: `card_${Date.now()}_${uid().slice(-8)}_${i}`, packId,
        cardName: String(card.cardName).trim(), name: String(card.cardName).trim(),
        rarity: input.packType === 'mystery' ? (['secret','god'].includes(card.rarity) ? 'secret' : ['rare','ultra'].includes(card.rarity) ? 'rare' : 'common') : card.rarity,
        pullChance: input.packType === 'mystery' ? ((quantity / Math.max(1, totalMysteryQuantity)) * 100) : (Number(card.pullChance) || 0),
        estimatedValue: Number(card.estimatedValue) || 0, value: Number(card.estimatedValue) || 0,
        cardImageUrl: card.cardImageUrl || null, imageUrl: card.cardImageUrl || null, sortOrder: i,
        quantity: input.packType === 'mystery' ? quantity : 0,
        originalQuantity: input.packType === 'mystery' ? Math.max(0, Number(card.originalQuantity ?? quantity)) : 0,
      }, CARD_COLUMNS);
...
app.delete('/admin/packs/:id', async c => {
  try {
    ...
    const result = await transaction(async client => {
      ...
      const history = await client.query(`SELECT EXISTS(SELECT 1 FROM packs_opened WHERE pack_id=$1) AS opened, EXISTS(SELECT 1 FROM pack_odds_versions WHERE pack_id=$1) AS odds`, [packId]);
      const hasHistory = Boolean(history.rows[0]?.opened || history.rows[0]?.odds);
      if (hasHistory) {
        const data = { ...(pack.data && typeof pack.data === 'object' ? pack.data : {}), adminDeleted: true, adminDeletedBy: adminUserId, adminDeletedAt: new Date().toISOString() };
