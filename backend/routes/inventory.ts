...
const inventoryRead = async (c:any) => {
  const userId = await auth(c); if (typeof userId !== 'string') return userId;
  try {
    const rows = await query(`SELECT id, card_id, card_name, rarity, value, emoji, locked, favorite, sold, is_locked, is_favorite, card_image_url, pack_name, created_at, data FROM inventory WHERE user_id=$1 AND COALESCE(sold,0)=0 ORDER BY created_at DESC`, [userId]);
    return c.json(rows.map((r:any) => ({ ...r, cardId:r.card_id, cardName:r.card_name, isLocked:Number(r.is_locked ?? r.locked ?? 0), isFavorite:Number(r.is_favorite ?? r.favorite ?? 0), cardImageUrl:r.card_image_url, packName:r.pack_name, createdAt:r.created_at, sold:Number(r.sold||0) })));
  } catch (e:any) { console.error('[inventory/list]',e); return c.json({error:'Failed to load inventory'},500); }
};
app.get('/inventory', inventoryRead);
app.get('/api/inventory', inventoryRead);
...
app.post('/inventory/sell-all',async c=>{
  ...
  try{
    const result=await transaction(async client=>{
      const cards=await client.query('SELECT * FROM inventory WHERE user_id=$1 AND COALESCE(sold,0)=0 AND COALESCE(locked,0)=0 FOR UPDATE',[userId]); if(!cards.rowCount)return null;
      const total=cards.rows.reduce((s:number,r:any)=>s+Number(r.value||0),0); const ids=cards.rows.map((r:any)=>r.id); const sourceId=`sellall_${ids.join('_').slice(0,80)}`;
