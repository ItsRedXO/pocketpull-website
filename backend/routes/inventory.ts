...
app.post('/inventory/sell',async c=>{
  ...
  try{
    const result=await transaction(async client=>{
      ...
      if(updated.rowCount !== 1)return {kind:'sold' as const};
    ...
    return c.json({success:true,inventoryId,value:result.value,cardValue:result.value,balance:result.balance,newBalance:result.balance});
...
app.post('/inventory/sell-all',async c=>{
  ...
  try{
    const result=await transaction(async client=>{
      ...
      await client.query('UPDATE inventory SET sold=1 WHERE user_id=$1 AND id=ANY($2::text[]) AND COALESCE(sold,0)=0',[userId,ids]);
