...
// NOTE: GET /admin/logs used to be handled here too, but that route is
// permanently shadowed by the identical one in adminLogs.ts (mounted earlier
// in backend/index.ts), so it was removed as dead code.
...
// NOTE: POST /admin/logs/action used to be handled here too, but that route
// is permanently shadowed by the identical one in adminLogs.ts (mounted
// earlier in backend/index.ts), so it was removed as dead code.
