...
import { getPack, listPackCards } from '../db/repositories/packs';
...
app.post('/open-pack', async (c) => {
  ...
  try {
    ...
    await recordPackOddsVersion(c.env as any, { contentHash: oddsVersionHash, packId, oddsJson, cardCount: dbCards.length });

    const clientSeed = `cs_${uid()}`;
    const nonceResult = await getDb(c.env as any).query(
      `INSERT INTO user_nonces (user_id, pack_nonce) VALUES ($1, 1)
       ...
       RETURNING pack_nonce`, [userId]);
    const nonce = Number(nonceResult.rows[0].pack_nonce);
    const rollValue = await computeRoll(serverSeed, clientSeed, nonce);
    const selectedCard = dbCards[selectCardIndex(rollValue, dbCards)];
    const inventoryId = `inv_${uid()}`;

    const result = await finalizePackOpen(c.env as any, {
      userId, recipientId: getRewardUserId(userId, false), pack, card: selectedCard,
      inventoryId, packOpeningId: `po_${uid()}`, walletTransactionId: `wtx_${uid()}`, transactionId: `txn_${uid()}`,
      clientSeed, nonce, rollValue, serverSeedHash: actualSeedHash, oddsVersionHash,
      emoji: RARITY_EMOJIS[selectedCard.rarity || 'common'] || '🃏', now,
    });
    if (!result.success) return c.json({ error: result.error || 'Failed to open pack' }, result.conflict ? 409 : 400);

    return c.json({ success: true, card: { name: selectedCard.cardName, rarity: selectedCard.rarity || 'common', value: selectedCard.estimatedValue, emoji: RARITY_EMOJIS[selectedCard.rarity || 'common'] || '🃏', imageUrl: selectedCard.cardImageUrl }, inventoryId, newBalance: result.balanceAfter, newMatchedBalance: result.matchedAfter });
  } catch (err: any) {
