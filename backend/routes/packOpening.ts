...
app.post('/open-pack', async (c) => {
  ...
  try {
    ...
    const [packRows, userRows] = await Promise.all([
      query('SELECT * FROM packs_catalog WHERE id=$1 LIMIT 1', [packId]),
      query('SELECT * FROM users WHERE id=$1 LIMIT 1', [userId]),
    ]);
    const pack: any = packRows[0];
    ...
    if (cooldownHours > 0) {
      ...
      if (rows[0]?.last_opened_at) {
        const diff = (Date.now() - new Date(rows[0].last_opened_at).getTime()) / 3600000;
        if (diff < cooldownHours) return c.json({ error: `Cooldown active: ${Math.ceil(cooldownHours - diff)}h remaining` }, 400);
      }
    }
    ...
    if (isMysteryPack) {
      const totalUnits = cards.reduce((sum, card) => sum + num(card.quantity), 0);
      cards = cards.map(card => ({ ...card, pullChance: totalUnits > 0 ? (num(card.quantity) / totalUnits) * 100 : 0 }));
    }
    ...
    const nonce = num(nonceRows[0]?.nonce);
    if (!nonce) return c.json({ error: 'Provably fair system error — nonce persistence failed. Please try again.' }, 500);
    ...
    if (isMysteryPack) {
      const claimed = await query('UPDATE pack_cards SET quantity=quantity-1 WHERE id=$1 AND quantity>0 RETURNING id', [picked.id]);
      if (!claimed.length) return c.json({ error: 'That Mystery Pack card just sold out. Please try again.' }, 409);
      await query('UPDATE packs_catalog SET current_quantity=(SELECT COALESCE(SUM(quantity),0) FROM pack_cards WHERE pack_id=$1) WHERE id=$1', [packId]);
    } else if (quantityLimit > 0) {
      const updated = await query('UPDATE packs_catalog SET current_quantity=GREATEST(current_quantity-1,0) WHERE id=$1 AND current_quantity>0 RETURNING current_quantity', [packId]);
      if (!updated.length) return c.json({ error: 'Pack is sold out' }, 409);
    }
    ...
    if (!walletResult.success) {
      ...
      return c.json({ error: `Failed to deduct balance: ${walletResult.error}` }, 400);
    }
