...
app.post('/exchanger/trade', async (c) => {
  ...
  try {
    ...
    if (offerInventoryIds.length > 100 || receivePackCardIds.length > 100) {
      return c.json({ error: 'Too many cards in one exchange' }, 400);
    }
    if (new Set(offerInventoryIds).size !== offerInventoryIds.length || new Set(receivePackCardIds).size !== receivePackCardIds.length) {
      return c.json({ error: 'Duplicate cards are not allowed' }, 400);
    }
    ...
    const result = await transaction(async client => {
      ...
      const offeredRows = await client.query(
        ...
        [userId, ...offerInventoryIds]
      );
      if (offeredRows.rowCount !== offerInventoryIds.length) {
      ...
      const receiveRows = await client.query(
        `SELECT * FROM pack_cards WHERE id IN (${receivePlaceholders})`,
        receivePackCardIds
      );
      ...
      for (const pc of receiveCards) {
        ...
        await client.query(
          ...
          [
            ...
            pc.card_name,
            pc.rarity,
            Number(pc.estimated_value ?? pc.value ?? 0),
            RARITY_EMOJIS[pc.rarity] || '🃏',
            pc.card_image_url || pc.image_url || null,
            null,
          ]
        );
        addedCards.push({
          ...
          rarity: pc.rarity,
          value: Number(pc.estimated_value ?? pc.value ?? 0),
          emoji: RARITY_EMOJIS[pc.rarity] || '🃏',
          cardImageUrl: pc.card_image_url || pc.image_url || null,
          packName: null,
          isLocked: false,
        });
      }
      ...
      return { kind: 'ok' as const, removedCardIds: offerInventoryIds, addedCards, refund, newBalance, offerTotal, receiveTotal };
    });
