...
test('battle recovery refunds use participant-specific idempotency sources in one transaction', async () => {
  ...
  assert.match(source, /const refundSource = `\$\{battleId\}:refund:\$\{human\.user_id\}`/);
  assert.match(source, /battle_entry_refund/);
  assert.match(source, /inventory_count/);
  assert.match(source, /audit_count/);
  assert.match(source, /type='battle_entry'/);
});
