-- PocketPull PostgreSQL migration 001
-- Provider-agnostic: works with any standard PostgreSQL 14+ instance.
-- Do not run against production until the migration has been reviewed and a backup exists.
...
CREATE TABLE IF NOT EXISTS packs_catalog (
  ...
  cooldown_hours NUMERIC(10,2) NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_packs_catalog_active ON packs_catalog(is_active);
...
CREATE INDEX IF NOT EXISTS idx_inventory_user_locked ON inventory(user_id, is_locked);
...
CREATE TABLE IF NOT EXISTS wallet_transactions (
  ...
  matched_before NUMERIC(18,2) NOT NULL,
  matched_after NUMERIC(18,2) NOT NULL,
  source_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_server_seeds_status_created ON server_seeds(status, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_pack_odds_versions_pack ON pack_odds_versions(pack_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_packs_opened_user_created ON packs_opened(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_packs_opened_pack_created ON packs_opened(pack_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS upgrader_multiplier_settings (
  multiplier NUMERIC(12,4) PRIMARY KEY,
  max_chance NUMERIC(8,4) NOT NULL DEFAULT 75,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- The battle subsystem is intentionally added in a follow-up migration after
-- its full field usage is mapped. This avoids silently losing battle-specific
-- state during the initial database cutover.
