...
app.get('/battles/stats', async (c) => {
  ...
  try {
    const activeBattles = await blink.db.battles.list({ where: { status: { in: ['waiting', 'live'] }, isPublic: 1 }, limit: 100 });
    return c.json({ success: true, liveBattles: activeBattles?.length || 0, packsOpened: getDailyPacksOpened(), timestamp: new Date().toISOString() });
  } catch (err: any) {
...
app.get('/referrals', async (c) => {
  ...
  try {
    const referredUsers = await blink.db.users.list({ where: { referredById: userId }, orderBy: { createdAt: 'desc' }, limit, offset });
    ...
    const data = await Promise.all((referredUsers as any[]).map(async (u) => {
      ...
      if (u.referralRewardPaid || Number(u.referralRewardPaid) > 0) {
        status = 'Reward Paid'; deposited = true;
      } else {
        const qualifyingDeposit = await blink.db.transactions.exists({ where: { userId: u.id, type: 'deposit', amount: { gte: 5 } } });
        if (qualifyingDeposit) { status = 'Deposit Pending'; deposited = true; }
        else {
          const anyDeposit = await blink.db.transactions.exists({ where: { userId: u.id, type: 'deposit' } });
