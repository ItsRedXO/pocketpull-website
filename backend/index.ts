...
import adminLogsGuardRoutes from './routes/adminLogsGuard';
...
const app = new Hono();
app.use('*', cors());
app.get('/health', async (c) => { try { await query('SELECT 1'); return c.json({ status: 'ok', database: 'postgresql', time: new Date().toISOString(), version: 'v4-postgresql' }); } catch (error) { console.error('[health] PostgreSQL unavailable:', error); return c.json({ status: 'degraded', database: 'unavailable', time: new Date().toISOString(), version: 'v4-postgresql' }, 503); } });

app.route('/', publicDbProxyRoutes); app.route('/', supportDbProxyRoutes); app.route('/', userLookupRoutes); app.route('/', userDbProxyRoutes); app.route('/', dbProxyRoutes);
app.route('/', adminAuthRoutes); app.route('/', adminUsersRoutes); app.route('/', adminPacksRoutes); app.route('/', stripeRoutes); app.route('/', coinbaseRoutes); app.route('/', packOpeningRoutes); app.route('/', upgraderRoutes); app.route('/', exchangerRoutes); app.route('/battles', battleRoutes);
