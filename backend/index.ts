...
function getDailyPacksOpened(): number { const pacificNow = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Los_Angeles' })); const start = new Date(pacificNow.getFullYear(), pacificNow.getMonth(), pacificNow.getDate()); const elapsed = pacificNow.getTime() - start.getTime(); const dateSeed = `${start.getFullYear()}-${start.getMonth()+1}-${start.getDate()}`; const target = hashDateToTarget(dateSeed); let t = Math.min(elapsed / 86400000, 1); t = t < 0.5 ? 4*t*t*t : 1-Math.pow(-2*t+2,3)/2; return Math.floor(target*t); }

app.get('/battles/stats', async (c) => { try { return c.json({ success: true, liveBattles: await countLivePublicBattles(c.env as any), packsOpened: getDailyPacksOpened(), timestamp: new Date().toISOString() }); } catch (err: any) { return c.json({ error: 'Failed to fetch battle stats' }, 500); } });
...
app.get('/referrals', async (c) => {
  ...
  const page = Math.max(1, Number(c.req.query('page') || 1)); const limit = 10;
  try {
    ...
    const data = await Promise.all(result.rows.map(async (u: any) => {
      ...
      return { id:u.id, username:u.username||u.display_name||'Trainer', email:u.is_deleted?'Released':(u.email||'Hidden'), status:u.referral_reward_paid?'Reward Paid':qualifying?'Deposit Pending':'Signed Up', deposited:qualifying||anyDeposit, createdAt:u.created_at };
    }));
