...
app.get('/health', async (c) => {
  try { await query('SELECT 1'); return c.json({ status: 'ok', database: 'postgresql', time: new Date().toISOString(), version: 'v4-postgresql' }); }
  catch (error) { console.error('[health] PostgreSQL unavailable:', error); return c.json({ status: 'degraded', database: 'unavailable', time: new Date().toISOString(), version: 'v4-postgresql' }, 503); }
});
