...
export async function createCashout(env: DbEnv, input: CreateCashoutInput) {
  ...
  try {
    ...
    const cardsResult = await client.query<any>('SELECT id,card_id,card_name,rarity,value,emoji,card_image_url,pack_name,is_locked FROM inventory WHERE id=ANY($1::text[]) AND user_id=$2 FOR UPDATE', [input.cardIds, input.userId]);
    ...
    const snapshots: CashoutCard[] = cards.map((card: any) => ({ card_id: card.card_id, card_name: card.card_name, rarity: card.rarity, value: Number(card.value || 0), card_image_url: card.card_image_url || '', pack_name: card.pack_name || '', emoji: card.emoji || '🃏' }));
    const totalValue = snapshots.reduce((sum, card) => sum + card.value, 0);
    const result = await client.query<any>(`
      INSERT INTO cashout_requests (id,user_id,username,confirmation_number,status,total_value,total_cards,cards_json,shipping_name,shipping_address,shipping_city,shipping_state,shipping_zip,shipping_country,notes,id_image_url,created_at,updated_at)
      VALUES ($1,$2,$3,$4,'pending',$5,$6,$7::jsonb,$8,$9,$10,$11,$12,$13,$14,$15,NOW(),NOW())
      RETURNING id,confirmation_number,total_value,total_cards,cards_json,username`, [
        input.id, input.userId, user.username || user.display_name || 'Trainer', input.confirmationNumber,
        totalValue, snapshots.length, JSON.stringify(snapshots), input.shipping.name, input.shipping.address,
        input.shipping.city, input.shipping.state, input.shipping.zip, input.shipping.country || 'US',
        `Email: ${input.shipping.email || user.email || ''} | Phone: ${input.shipping.phone || ''}`, input.idImageUrl,
      ]);
    await client.query('DELETE FROM inventory WHERE id=ANY($1::text[]) AND user_id=$2', [input.cardIds, input.userId]);
    await client.query('COMMIT');
    return { request: result.rows[0], cards: snapshots, username: user.username || user.display_name || 'Trainer', email: input.shipping.email || user.email || '' };
...
export async function partialFulfillCashout(env: DbEnv, input: { cashoutId: string; fulfilledIndices: number[]; trackingNumber?: string }) {
  ...
  try {
    ...
    for (const card of returnedCards) await client.query('INSERT INTO inventory (id,user_id,card_id,card_name,rarity,value,emoji,is_favorite,created_at,card_image_url,pack_name,is_locked,battle_id) VALUES ($1,$2,$3,$4,$5,$6,$7,FALSE,NOW(),$8,$9,FALSE,NULL)', [`inv_${crypto.randomUUID()}`, req.user_id, card.card_id, card.card_name, card.rarity || 'common', card.value, card.emoji || '🃏', card.card_image_url || null, card.pack_name || null]);
