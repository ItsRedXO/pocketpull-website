 ...
 try{
  ...
  for(const p of players){const cards:any[]=[]; const ai=Boolean(p.is_ai||p.isAi); const userId=p.user_id;
   for(const pack of packs){pullIndex++; const pool=cardMap.get(pack.id)||[]; if(!pool.length)continue;
    ...
    const nonce=ai?pullIndex:Number(nonceQ?.rows[0]?.pack_nonce); const clientSeed=`cs_bt_${battleId}_${p.id}_${pullIndex}`; const roll=await computeRoll(serverSeed,clientSeed,nonce);
  ...
  const winningHumans=results.filter(r=>r.isWinner&&!r.isAi);
  let rewardIndex=0;
  for(const r of results){await client.query(`UPDATE battle_players SET cards_json=$2::jsonb,total_value=$3,is_winner=$4 WHERE id=$1`,[r.playerId,JSON.stringify(r.cards),r.totalValue,r.isWinner?1:0]);
   for(const card of r.cards){let recipient:BattleResult|null=null;
    if(mode==='shared'||draw) recipient=r; else if(teamMode) recipient=winningHumans.length?winningHumans[rewardIndex++%winningHumans.length]:null; else recipient=winner;
