...
const BOT_WIN_ROLL_THRESHOLD=51.02;
...
function sharedDistribute(results:BattleResult[]):void{
 if(results.length<2)return;
 const all=results.flatMap(r=>r.cards); const per=all.length/results.length;
 if(!Number.isInteger(per))throw new Error(`Shared battle card count ${all.length} cannot be evenly divided among ${results.length} players`);
 const target=all.reduce((s,c)=>s+Number(c.value||0),0)/results.length;
 const buckets=results.map(r=>({r,cards:[] as any[],total:0}));
 for(const card of [...all].sort((a,b)=>Number(b.value)-Number(a.value))){const eligible=buckets.filter(b=>b.cards.length<per);eligible.sort((a,b)=>a.total-b.total);eligible[0].cards.push(card);eligible[0].total+=Number(card.value||0);}
 let improved=true;
 while(improved){improved=false;let maxDev=Math.max(...buckets.map(b=>Math.abs(b.total-target)));
  outer:for(let i=0;i<buckets.length;i++)for(let j=i+1;j<buckets.length;j++)for(let ai=0;ai<buckets[i].cards.length;ai++)for(let bj=0;bj<buckets[j].cards.length;bj++){
   const a=buckets[i],b=buckets[j],ca=a.cards[ai],cb=b.cards[bj]; const na=a.total-Number(ca.value)+Number(cb.value),nb=b.total-Number(cb.value)+Number(ca.value);
   let nd=0;for(let k=0;k<buckets.length;k++){const t=k===i?na:k===j?nb:buckets[k].total;nd=Math.max(nd,Math.abs(t-target));}
   if(nd<maxDev-0.0001){a.cards[ai]=cb;b.cards[bj]=ca;a.total=na;b.total=nb;improved=true;break outer;}
  }
 }
 buckets.forEach(b=>{b.r.cards=b.cards;b.r.totalValue=Math.round(b.total*100)/100;});
}
...
export async function executeBattle(env:DbEnv,battleId:string,serverSeed:string){
 ...
 try{
  ...
  const oddsHashes:Record<string,string>={}; for(const pid of packIds){const pool=cardMap.get(pid)||[];if(pool.length)oddsHashes[pid]=await sha256(buildOddsSnapshot(pool));}
  ...
  for(const p of players){const cards:any[]=[];const ai=Boolean(p.is_ai||p.isAi);const userId=p.user_id;
   for(const pack of packs){pullIndex++;const pool=cardMap.get(pack.id)||[];if(!pool.length)continue;
    ...
    const nonce=ai?pullIndex:Number(nonceQ?.rows[0]?.pack_nonce);const clientSeed=`cs_bt_${battleId}_${p.id}_${pullIndex}`;const roll=await computeRoll(serverSeed,clientSeed,nonce);const raw=pool[selectCardIndex(roll,pool)];const rarity=raw.rarity||'common';const oddsHash=oddsHashes[pack.id]||'';
  ...
  const mode=battle.mode||'standard';const teamMode=Number(battle.team_mode||0)>0;let winner:BattleResult|null=null;let draw=false;let winningTeam:string|null=null;
  if(mode==='shared'){results.forEach(r=>r.isWinner=true);sharedDistribute(results);}
  else if(teamMode){const left=results.filter(r=>r.teamSide==='left').reduce((s,r)=>s+r.totalValue,0);const right=results.filter(r=>r.teamSide==='right').reduce((s,r)=>s+r.totalValue,0);if(Math.round(left*100)===Math.round(right*100))draw=true;else{winningTeam=left>right?'left':'right';results.forEach(r=>r.isWinner=r.teamSide===winningTeam);winner=results.find(r=>r.teamSide===winningTeam)||null;}}
  else if(mode==='underdog'){const hasBot=results.some(r=>r.isAi),hasHuman=results.some(r=>!r.isAi);let faction:null|'bot'|'human'=null;if(hasBot&&hasHuman){const diceSeed=`dice_${battleId}`,diceNonce=pullIndex+1,diceRoll=await computeRoll(serverSeed,diceSeed,diceNonce);faction=diceRoll<BOT_WIN_ROLL_THRESHOLD?'bot':'human';await client.query(`INSERT INTO battle_pull_audits(id,battle_id,battle_player_id,user_id,pack_id,pack_name,card_name,rarity,cost,client_seed,nonce,roll_value,server_seed_hash,odds_version_hash,is_bot,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,'system',0,$8,$9,$10,$11,'',FALSE,NOW())`,[id('bpa_dice'),battleId,'system','system','system','[BATTLE_DICE]',`Underdog dice — faction: ${faction}`,diceSeed,diceNonce,diceRoll,actualHash]);}const pool=faction==='bot'?results.filter(r=>r.isAi):faction==='human'?results.filter(r=>!r.isAi):results;const min=Math.min(...pool.map(r=>r.totalValue));const tops=pool.filter(r=>Math.round(r.totalValue*100)===Math.round(min*100));if(tops.length===pool.length&&pool.length>1)draw=true;else{winner=tops[0]||pool[0]||null;if(winner)winner.isWinner=true;}}
  else {const max=Math.max(...results.map(r=>r.totalValue));const tops=results.filter(r=>Math.round(r.totalValue*100)===Math.round(max*100));if(tops.length===results.length&&results.length>1)draw=true;else{winner=tops[0]||null;if(winner)winner.isWinner=true;}}
  const winningHumans=results.filter(r=>r.isWinner&&!r.isAi);let rewardIndex=0;
  for(const r of results){await client.query(`UPDATE battle_players SET cards_json=$2::jsonb,total_value=$3,is_winner=$4 WHERE id=$1`,[r.playerId,JSON.stringify(r.cards),r.totalValue,r.isWinner]);for(const card of r.cards){let recipient:BattleResult|null=null;if(mode==='shared'||draw)recipient=r;else if(teamMode)recipient=winningHumans.length?winningHumans[rewardIndex++%winningHumans.length]:null;else recipient=winner;if(recipient&&!recipient.isAi)await client.query(`INSERT INTO inventory(id,user_id,card_id,card_name,rarity,value,emoji,is_favorite,created_at,card_image_url,pack_name,is_locked,battle_id) VALUES($1,$2,$3,$4,$5,$6,$7,FALSE,NOW(),$8,$9,FALSE,$10)`,[id('inv'),recipient.userId,card.cardId,card.name,card.rarity,card.value,card.emoji,card.imageUrl,card.packName,battleId]);}}
