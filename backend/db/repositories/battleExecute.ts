...
export async function executeBattle(env:DbEnv,battleId:string,serverSeed:string){
 ...
 try{
  ...
  const results:any[]=[]; let pullIndex=0;
  for(const p of players){const cards:any[]=[]; const ai=Boolean(p.is_ai||p.isAi); const userId=p.user_id;
   for(const pack of packs){pullIndex++; const pool=cardMap.get(pack.id)||[]; if(!pool.length)continue;
    ...
    const card={id:id('po'),name:raw.card_name,rarity,value:Number(raw.estimated_value||0),imageUrl:raw.card_image_url||null,packId:pack.id,packName:pack.name,emoji:RARITY_EMOJIS[rarity]||'🃏',clientSeed,nonce,rollValue:roll,serverSeedHash:actualHash,oddsVersionHash:oddsHash,isBot:ai}; cards.push(card);
  ...
  const mode=battle.mode||'standard'; const teamMode=Number(battle.team_mode||0)>0; let winner:any=null; let draw=false;
  ...
  for(const r of results){await client.query(`UPDATE battle_players SET cards_json=$2::jsonb,total_value=$3,is_winner=$4 WHERE id=$1`,[r.playerId,JSON.stringify(r.cards),r.totalValue,r.isWinner?1:0]);
   // Only human players receive physical inventory. Shared mode keeps each player's assigned share.
   if(!r.isAi){for(const card of r.cards){const inv=id('inv');await client.query(`INSERT INTO inventory(id,user_id,card_id,card_name,rarity,value,emoji,is_favorite,created_at,card_image_url,pack_name,is_locked,battle_id) VALUES($1,$2,$3,$4,$5,$6,$7,FALSE,NOW(),$8,$9,FALSE,$10)`,[inv,r.userId,card.cardId||null,card.name,card.rarity,card.value,card.emoji,card.imageUrl,card.packName,battleId]);}}
  }
