...
export async function executeBattle(env:DbEnv,battleId:string,serverSeed:string){
 ...
 try{
  ...
  else if(mode==='underdog'){const hasBot=results.some(r=>r.isAi),hasHuman=results.some(r=>!r.isAi);let faction:null|'bot'|'human'=null;if(hasBot&&hasHuman){const diceSeed=`dice_${battleId}`,diceNonce=pullIndex+1,diceRoll=await computeRoll(serverSeed,diceSeed,diceNonce);faction=diceRoll<BOT_WIN_ROLL_THRESHOLD?'bot':'human';}
    const pool=faction==='bot'?results.filter(r=>r.isAi):faction==='human'?results.filter(r=>!r.isAi):results;const min=Math.min(...pool.map(r=>r.totalValue));const tops=pool.filter(r=>Math.round(r.totalValue*100)===Math.round(min*100));if(tops.length===pool.length&&pool.length>1)draw=true;else{winner=tops[0]||pool[0]||null;if(winner)winner.isWinner=true;}}
