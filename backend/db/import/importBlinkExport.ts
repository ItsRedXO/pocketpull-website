  ...
  await transaction(async client => {
    for (const table of IMPORT_ORDER) {
      ...
      for (const rawRow of rows) {
        const row = withUnknownFields(table, rawRow);
        // Only send fields that actually existed in the Blink export. This preserves
        // PostgreSQL defaults for fields added during the migration that Blink never had.
