...
test('Packs admin tab does not order packs by a missing sortOrder column', () => {
  ...
  assert.match(source, /packsCatalog\.list\(\{\s*\/\/ PostgreSQL: packs_catalog has no sort_order column\./);
...
test('Email admin tab orders by createdAt and reads legacy fields from row data', () => {
  ...
  assert.match(source, /sentAt:\s*String\(r\.sentAt \?\? r\.data\?\.sentAt \?\? r\.createdAt/);
});
