...
export const AdminDashboard: React.FC<Props> = ({ onLogout }) => {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    { id: 'packs', label: 'Packs', icon: <Package size={14} /> }, { id: 'users', label: 'Users', icon: <Users size={14} /> },
    { id: 'pulls', label: 'Recent Pulls', icon: <History size={14} /> }, { id: 'stats', label: 'Stats', icon: <BarChart3 size={14} /> },
    { id: 'support', label: 'Support Chats', icon: <MessageCircle size={14} /> }, { id: 'cashouts', label: 'Cash Outs', icon: <Banknote size={14} /> },
    { id: 'logs', label: 'Logs', icon: <FileText size={14} /> }, { id: 'emails', label: 'Emails', icon: <Mail size={14} /> },
  ...
  return <div className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(155,92,255,0.06) 0%, #07080e 55%)' }}>
    <header className="sticky top-0 z-30 min-h-14 flex items-center justify-between gap-3 px-3 sm:px-6 py-2 border-b border-white/5" style={{ background: 'rgba(7,8,14,0.96)', backdropFilter: 'blur(20px)' }}>
      ...
      <div className="flex items-center gap-1.5 shrink-0"><button onClick={() => { window.location.href = '/'; }} aria-label="Back to Site" className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-[10px] sm:text-[11px] font-bold uppercase tracking-wider" style={{ color: '#00c8ff', background: 'rgba(0,200,255,0.08)', border: '1px solid rgba(0,200,255,0.2)' }}><Home size={13} /> <span className="hidden sm:inline">Back to Site</span></button><button onClick={onLogout} aria-label="Logout" className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-red-400/60 hover:text-red-400 hover:bg-red-400/10 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider"><LogOut size={13} /> <span className="hidden sm:inline">Logout</span></button></div>
    </header>
    ...
    <main className="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6 min-w-0"><AnimatePresence mode="wait"><motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>
      {tab === 'packs' && <PacksTab showToast={showToast} />}{tab === 'users' && <UsersTab showToast={showToast} />}{tab === 'pulls' && <PullsTab />}{tab === 'stats' && <StatsTabFixed />}{tab === 'support' && <SupportChatsTab showToast={showToast} />}{tab === 'cashouts' && <CashOutsTabFixed showToast={showToast} />}{tab === 'logs' && <LogsTabFixed />}{tab === 'emails' && <EmailsTab showToast={showToast} />}{tab === 'provably-fair' && <ProvablyFairTab showToast={showToast} />}
