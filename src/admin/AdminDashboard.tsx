...
import { Package, LogOut, Shield, ShieldCheck, CheckCircle, AlertCircle, Users, BarChart3, History, Home, MessageCircle, Banknote, FileText, Mail, Swords, SlidersHorizontal, Ticket } from 'lucide-react';
...
import { AdminPokeBrawlPanel } from './AdminPokeBrawlPanel';
import { SiteSettingsTab } from './SiteSettingsTab';
import { CodesAndDealsTab } from './CodesAndDealsTab';

type Tab = 'packs' | 'users' | 'pulls' | 'stats' | 'support' | 'cashouts' | 'logs' | 'emails' | 'provably-fair' | 'brawl' | 'site-settings' | 'codes-deals';
...
export const AdminDashboard: React.FC<Props> = ({ onLogout }) => {
  ...
  return <div className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(155,92,255,0.06) 0%, #07080e 55%)' }}>
    ...
    <div className="sticky top-14 z-20 border-b border-white/5 px-2 sm:px-6" style={{ background: 'rgba(7,8,14,0.94)', backdropFilter: 'blur(16px)' }}><div className="max-w-[1900px] mx-auto flex gap-1 overflow-x-auto overscroll-x-contain snap-x snap-mandatory" role="tablist">{tabs.map(t => <button key={t.id} onClick={() => setTab(t.id)} role="tab" aria-selected={tab === t.id} className="relative flex items-center gap-1.5 px-3 sm:px-4 py-3 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider whitespace-nowrap shrink-0 snap-start" style={{ color: tab === t.id ? '#fff' : 'rgba(255,255,255,0.3)' }}>{t.icon} {t.label}{tab === t.id && <motion.div layoutId="tab-indicator" className="absolute bottom-0 left-1 right-1 h-0.5" style={{ background: 'linear-gradient(90deg, #9b5cff, #00c8ff)' }} />}</button>)}</div></div>
    <main className="max-w-[1900px] mx-auto px-3 sm:px-4 py-4 sm:py-6 min-w-0 overflow-x-hidden"><AnimatePresence mode="wait"><motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>{tab === 'packs' && <PacksTab showToast={showToast} />}{tab === 'users' && <UsersTab showToast={showToast} />}{tab === 'pulls' && <PullsTab />}{tab === 'stats' && <StatsTabFixed />}{tab === 'support' && <SupportChatsTabFixed showToast={showToast} />}{tab === 'cashouts' && <CashOutsTabFixed showToast={showToast} />}{tab === 'logs' && <LogsTabFixed />}{tab === 'emails' && <EmailsTab showToast={showToast} />}{tab === 'provably-fair' && <ProvablyFairTab showToast={showToast} />}{tab === 'brawl' && <AdminPokeBrawlPanel showToast={showToast} />}{tab === 'site-settings' && <SiteSettingsTab showToast={showToast} />}{tab === 'codes-deals' && <CodesAndDealsTab showToast={showToast} />}</motion.div></AnimatePresence></main>
