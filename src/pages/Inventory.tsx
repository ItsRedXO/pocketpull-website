...
export const Inventory: React.FC<{ onDepositOpen?: () => void; onProfileOpen?: () => void }> = ({ onDepositOpen, onProfileOpen }) => {
  ...
  const setRawCards = useCallback((updater: (prev: RawCard[]) => RawCard[]) => { qc.setQueryData([...INVENTORY_QUERY_KEY, user?.id], updater); }, [qc, user?.id]);
  ...
  const statsValues = useMemo(() => ({ totalCards: rawCards.length, totalValue: rawCards.reduce((a, c) => a + Number(c.value), 0), rarePlus: rawCards.filter(c => ['rare', 'ultra', 'secret', 'god', 'rainbow'].includes(c.rarity)).length, favorites: rawCards.filter(c => Boolean(c.isFavorite)).length, lockedCount: rawCards.filter(c => Boolean(c.isLocked)).length, sellableCards: grouped.filter(c => !c.isLocked), sellableValue: rawCards.filter(c => !Boolean(c.isLocked)).reduce((a, c) => a + Number(c.value), 0) }), [rawCards, grouped]);
  ...
  const handleSell = async (card: InventoryCard) => { if (!user?.id || card.isLocked || selling) return; const id = card.allIds[0]; setSelling(card.cardId); setRawCards(p => p.filter(r => r.id !== id)); setSellMsg(`Selling ${card.cardName}...`); try { const result = await sellCard(id); qc.setQueryData([...INVENTORY_QUERY_KEY, user.id], (p: RawCard[] | undefined) => (p || []).filter(r => r.id !== id && Number((r as any).sold ?? 0) === 0)); updateBalance(result.newBalance); setSellMsg(`Sold ${card.cardName}!`); } catch (err: any) { setSellMsg(err.message || 'Failed to sell card.'); qc.invalidateQueries({ queryKey: [...INVENTORY_QUERY_KEY, user.id] }); } finally { setSelling(null); setSellConfirm(null); setTimeout(() => setSellMsg(null), 3000); } };
  const handleSellAll = async () => { if (!user?.id || !statsValues.sellableCards.length || sellingAll) return; setSellingAll(true); setSellMsg(`Selling ${statsValues.sellableCards.length} cards...`); try { const result = await sellAllCards(); setRawCards(p => p.filter(r => Boolean(r.isLocked))); updateBalance(result.newBalance); setSellMsg(`Sold ${result.count} cards!`); } catch (err: any) { setSellMsg(err.message || 'Failed to sell all cards.'); } finally { setSellingAll(false); setSellAllConfirm(false); setTimeout(() => setSellMsg(null), 4000); } };
