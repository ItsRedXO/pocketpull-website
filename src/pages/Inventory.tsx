...
export const Inventory: React.FC<{ onDepositOpen?: () => void; onProfileOpen?: () => void }> = ({ onDepositOpen, onProfileOpen }) => {
  ...
  const { data: rawCards = [], isLoading: loading, refetch: loadInventory } = useQuery<RawCard[]>({
    ...
    staleTime: 30_000, // 30s cache — realtime keeps it fresh
  });
  ...
  const grouped: InventoryCard[] = useMemo(() => rawCards.map(row => ({
    ...row,
  ...
  const toggleFavorite = async (card: InventoryCard) => {
    ...
    try {
      await favoriteCard(card.allIds[0], newFav);
    } catch (err) {
  ...
  const toggleLock = async (card: InventoryCard) => {
    ...
    try {
      await lockCard(card.allIds[0], newLocked);
    } catch (err) {
  ...
  const handleSell = async (card: InventoryCard) => {
    ...
    // Optimistic: remove card from view only (balance from backend)
    setSelling(card.cardId);
    setRawCards(prev => prev.filter(r => r.id !== idToRemove));
    qc.setQueryData([...INVENTORY_QUERY_KEY, user.id], (prev: RawCard[] | undefined) =>
      (prev || []).filter((r: RawCard) => r.id !== idToRemove)
    );
    setSellMsg(`Selling ${card.cardName} for ${card.value.toFixed(2)}...`);

    try {
      const result = await sellCard(idToRemove);
      // Confirm exact balance from backend — single source of truth
      updateBalance(result.newBalance);
      setSellMsg(`Sold ${card.cardName} for ${card.value.toFixed(2)}!`);
    } catch (err: any) {
      // Revert: restore card to cache, balance unchanged
  ...
  const handleSellAll = async () => {
    ...
    if (!user?.id || sellableCards.length === 0) return;
    if (sellingAll) return; // prevent double-clicks
    setSellingAll(true);
    // Show immediate feedback — confirm UI stays visible with processing state
    setSellMsg(`Selling ${sellableCards.length} cards...`);
    try {
      const result = await sellAllCards();
      // Remove all unlocked cards from cache (safe: API confirmed success)
      setRawCards(prev => prev.filter(r => Boolean(r.isLocked)));
      qc.setQueryData([...INVENTORY_QUERY_KEY, user.id], (prev: RawCard[] | undefined) =>
        (prev || []).filter((r: RawCard) => Boolean(r.isLocked))
      );
      updateBalance(result.newBalance);
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center space-y-4 px-4">
          <div className="text-6xl mb-4 text-[#00c8ff]/20">🔒</div>
          <h2 className="text-2xl font-display text-white uppercase tracking-tight">Sign In Required</h2>
          <p className="text-gray-400 text-sm max-w-xs mx-auto">Log in to view and manage your personal card collection.</p>
        </div>
      </div>
    );
  }
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="min-h-screen bg-[#0a0b0f] pb-24">
        <div className="container mx-auto px-4 py-12 max-w-7xl">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
            <InventoryHeader
              totalCards={statsValues.totalCards}
              totalValue={statsValues.totalValue}
              lockedCount={statsValues.lockedCount}
              sellableCount={statsValues.sellableCards.length}
              sellableValue={statsValues.sellableValue}
              sellAllConfirm={sellAllConfirm}
              sellingAll={sellingAll}
              sellMsg={sellMsg}
              onSetSellAllConfirm={setSellAllConfirm}
              onHandleSellAll={handleSellAll}
            />
            <div className="flex items-center gap-3">
              <button onClick={() => loadInventory()} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 transition-all disabled:opacity-40 font-bold text-xs uppercase tracking-widest">
