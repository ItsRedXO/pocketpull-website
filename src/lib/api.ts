...
async function get<T>(path:string):Promise<T>{ const res=await fetch(`${BACKEND_BASE}${path}`,{headers:await getAuthHeaders()}); const data=await res.json().catch(()=>({})); if(!res.ok)throw new Error(data?.error||`API error ${res.status}`); return data as T; }
async function post<T>(path:string,body:unknown):Promise<T>{ const res=await fetch(`${BACKEND_BASE}${path}`,{method:'POST',headers:await getAuthHeaders(),body:JSON.stringify(body)}); const data=await res.json().catch(()=>({})); if(!res.ok)throw new Error(data?.error||`API error ${res.status}`); return data as T; }
...
export const fetchReferrals=(page:number=1)=>get<FetchReferralsResult>(`/referrals?page=${page}`);
...
async function getAdminHeaders(){ const headers=await getAuthHeaders(); const pass=localStorage.getItem('pocketpull_admin_pass')||''; if(pass)headers['X-Admin-Secret']=pass; return headers; }
...
export const fetchAdminSeedStatus=()=>get<AdminSeedStatus>('/admin/provably-fair/status');
export const adminGenerateSeed=()=>post<GenerateSeedResult>('/admin/provably-fair/generate-seed',{});
export const adminCompleteRotation=(oldSeed:string)=>post<CompleteRotationResult>('/admin/provably-fair/complete-rotation',{oldSeed});
