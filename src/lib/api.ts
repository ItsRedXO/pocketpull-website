...
// Backend URL is deployment-specific. Keep the existing Blink backend as the
// transition fallback until the new Node/Postgres API is deployed.
const BACKEND_BASE = (import.meta.env.VITE_BACKEND_URL || 'https://b2nnhe2n.backend.blink.new').replace(/\/$/, '');
...
export const fetchReferrals = async (page: number = 1) => get<FetchReferralsResult>(`/referrals?page=${page}`);
...
export const fetchProvablyFairOpenings = async () => get<{ openings: ProvablyFairOpening[] }>('/provably-fair/my-openings');
export const fetchProvablyFairVerify = async (openingId: string) => get<ProvablyFairVerifyData>(`/provably-fair/verify/${encodeURIComponent(openingId)}`);
...
export const fetchProvablyFairUpgrades = async () => get<{ spins: ProvablyFairUpgrade[] }>('/provably-fair/my-upgrades');
export const fetchProvablyFairVerifyUpgrade = async (spinId: string) => get<ProvablyFairVerifyUpgradeData>(`/provably-fair/verify-upgrade/${encodeURIComponent(spinId)}`);

// ── Admin ────────────────────────────────────────────────────────────────
async function getAdminHeaders(): Promise<Record<string, string>> {
  const pass = localStorage.getItem('pocketpull_admin_pass') || '';
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  try { const token = await blink.auth.getValidToken(); if (token) headers.Authorization = `Bearer ${token}`; } catch { /* no token */ }
...
export const fetchAdminSeedStatus = async () => { const headers = await getAdminHeaders(); const res = await fetch(`${BACKEND_BASE}/admin/provably-fair/status`, { method: 'GET', headers }); const data = await res.json() as any; if (!res.ok) throw new Error(data?.error || `API error ${res.status}`); return data as AdminSeedStatus; };
export const adminGenerateSeed = async () => { const headers = await getAdminHeaders(); const res = await fetch(`${BACKEND_BASE}/admin/provably-fair/generate-seed`, { method: 'POST', headers }); const data = await res.json() as any; if (!res.ok) throw new Error(data?.error || `API error ${res.status}`); return data as GenerateSeedResult; };
export const adminCompleteRotation = async (oldSeed: string) => { const headers = await getAdminHeaders(); const res = await fetch(`${BACKEND_BASE}/admin/provably-fair/complete-rotation`, { method: 'POST', headers, body: JSON.stringify({ oldSeed }) }); const data = await res.json() as any; if (!res.ok) throw new Error(data?.error || `API error ${res.status}`); return data as CompleteRotationResult; };
